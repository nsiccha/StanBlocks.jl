...
slic_implementation(::Val{:diamonds}; N, Y, K, X, kwargs...) = begin
    # Stan model centers X in transformed data; reproduce in Julia
    Xmat = reduce(hcat, X)'  # N×K matrix
    Kc = K - 1
...
slic_implementation(::Val{:accel_splines}; N, Y, Ks, Xs, knots_1, Zs_1_1,
    ...
    # Stan model applies exp() elementwise in a loop; avoid by using exp(sigma_log)
...
@testset "posteriordb" begin
    map(PosteriorDB.posterior_names(pdb)) do posterior_name
