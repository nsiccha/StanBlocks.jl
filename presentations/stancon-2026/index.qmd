...
- one host language for data + models

:::
::: {.column width="48%"}

### The extra half-language

- what the selected AD stack can differentiate
- which mutation and aliasing patterns survive
- which types compile efficiently
- which backend-specific failures appear only at runtime

:::
::::

::: {.takeaway}
The model author can end up reasoning about the statistical program **and** the admissible AD subset of Julia.
:::

::: {.notes}
“1.5 languages” is a framing, not a language count. Julia is expressive; the
trouble is that differentiable Julia is often a moving subset with another set
of operational constraints. In this stack, Mooncake is the Julia AD backend;
Stan's path is separate and uses Stan Math through BridgeStan.
:::
...
1. **Julia is a great authoring language**, but Julia + AD can become 1.5 languages.
