...
tests = map(posterior_names) do posterior_name
    ...
    # inferred = try
    #     @inferred jlpdf(randn(LogDensityProblems.dimension(jlpdf)))
    #     true
    # catch e
    #     false
    # end
    # Serialization.serialize(cache_path, merge(rv, (;inferred)))
    ...
    dimension = if LogDensityProblems.dimension(stan_problem) == LogDensityProblems.dimension(jlpdf)
        n
    else
        "FAILED"
    end
    m = 200
    X = randn((n, m))
    jlpdfs = mapreduce(nan_on_error(jlpdf), vcat, eachcol(X)) 
