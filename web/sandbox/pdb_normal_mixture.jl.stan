...
real shifted(real x, int mu, int sigma) {
    return ((x - mu) / sigma);
}
}
data {
...
generated quantities {
    vector[y_n] y_likelihood = normal_lpdfs(y, (shifted(alpha, 0, 1) + (beta * x)), sigma);
    vector[y_n] y_gen = normal_vector_rng(y_n, (shifted(alpha, 0, 1) + (beta * x)), sigma);
}