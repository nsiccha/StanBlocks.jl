...
real my_add(real f, real g, real h) {
    return (f + (g * h));
}
}
data {
...
generated quantities {
    vector[y_n] y_likelihood = normal_lpdfs(y, my_add(mu, sigma, mu), sigma);
    vector[y_n] y_gen = normal_vector_rng(y_n, my_add(mu, sigma, mu), sigma);
}