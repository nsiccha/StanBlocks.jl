...
real sum_and_scale(real x, real y, real s) {
    return ((x + y) * s);
}
}
data {
...
generated quantities {
    vector[y_n] y_likelihood = normal_lpdfs(y, sum_and_scale(mu, 1.0, sigma), 1);
    vector[y_n] y_gen = normal_vector_rng(y_n, sum_and_scale(mu, 1.0, sigma), 1);
}