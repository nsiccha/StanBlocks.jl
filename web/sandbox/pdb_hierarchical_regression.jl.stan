...
vector hier_mean(real mu, real tau, vector offset) {
    return (mu + (tau * offset));
}
}
data {
    int y_n;
...
parameters {
    real mu_alpha;
    real mu_beta;
    real<lower=0.0> tau_alpha;
    real<lower=0.0> tau_beta;
    real alpha;
    real beta;
...
generated quantities {
    vector[y_n] y_likelihood = normal_lpdfs(y, hier_mean(alpha, beta, x), sigma);
    vector[y_n] y_gen = normal_vector_rng(y_n, hier_mean(alpha, beta, x), sigma);
}