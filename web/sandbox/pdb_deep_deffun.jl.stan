...
vector shift_and_scale(vector x, real m, real s) {
    return shift((x * s), m);
}
vector shift(vector x, real m) {
    return (x + m);
}
}
data {
...
generated quantities {
    vector[y_n] y_likelihood = normal_lpdfs(y, shift_and_scale(x, alpha, beta), sigma);
    vector[y_n] y_gen = normal_vector_rng(y_n, shift_and_scale(x, alpha, beta), sigma);
}