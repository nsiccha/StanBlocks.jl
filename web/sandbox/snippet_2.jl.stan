...
real my_complex_function(real f, real g, real h) {
    return (f + (g * h));
}
}
data {
...
generated quantities {
    vector[y_n] y_likelihood = normal_lpdfs(y, my_complex_function(mu, sigma, mu), sigma);
    vector[y_n] y_gen = normal_vector_rng(y_n, my_complex_function(mu, sigma, mu), sigma);
}